...
where $W_i^k$ is the weight matrix that transforms the input X to query, key and value. Where $X$ has the dim of $n_max$ x $d_{model}$ and W_i has the dim of $d_{model}$ x $d_k$ assuming $d_k = d_q = d_v$
